...
p.addParamValue('SpikeDuration',0.001,@(x) isnumeric(x) || isvector(x));
...
if islogical(spikes)
    ...
    if strcmpi(plotType,'vertline') || strcmpi(plotType,'horzline')
        ...
        if strcmpi(plotType,'vertline')
            ...
            %% Horizontal Lines
                if length(spikeDuration) == 1
                    spikeDurations = zeros(nTrials,1);
                    spikeDurations(:) = spikeDuration;
                else
                    spikeDurations = spikeDuration;
                end
            for trials = 1:nTrials
                ...
                trialXPoints = [ spikes{trials} + relSpikeStartTime;...
                    spikes{trials} + relSpikeStartTime + spikeDurations(trials);...
                    nanSeparator ];
                trialXPoints = trialXPoints(:);
