%%
function [ keep_direction, keep_angle, keep_wings ] = PD_direction4(step2Image, blobAreas, blobCenterPoints, blobMajorAxis, blobOrient, blobEcc, params)
    % init
    ...
    % constant params
    bodyColorMin = params{1};
    bodyColorMax = params{2};
    radiusRate = params{3};
    range = params{4};
    step = params{5};
    ...
    %
    wingImage = applyWingFilter(step2Image, bodyColorMin, bodyColorMax);
    
    % label image (for mask blob)
    img2 = imgaussfilt(step2Image,2);
    img2(img2>=bodyColorMax) = 255;
    img2 = 255 - img2;
