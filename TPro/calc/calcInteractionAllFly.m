%%
function interaction_data = calcInteractionAllFly(X, Y, dir, r, angleTh)
    flameMax = size(X, 1);
    flyNum = size(X, 2);
    ...
    for i = 1:flameMax
        ...
        [hhInt(i,:), haInt(i,:), hbInt(i,:), hx(i,:), hy(i,:), ax(i,:), ay(i,:)] = calcInteractionFrame(fx,fy,fdir,r,angleTh);
    end
    % count result
