    ...
    for data_th = 1:size(records,1)
        ...
        for i=1:10
            if i>length(keep_data_out)
                keep_data_out{i} = keep_data{i};
                assignCost_out(i) = assignCost(i);
            else
                if size(keep_data_out{i},1) > size(keep_data{i},1)
                    tmp = nan(size(keep_data_out{i},1),size(keep_data{i},2));
                    tmp(1:size(keep_data{i},1),:) = keep_data{i};
                    keep_data_out{i} = [keep_data_out{i}, tmp];
                elseif length(keep_data_out{i}) < length(keep_data{i})
                    tmp = nan(size(keep_data{i},1),size(keep_data_out{i},2));
                    tmp(1:size(keep_data_out{i},1),:) = keep_data_out{i};
                    keep_data_out{i} = [tmp,keep_data{i}];
                else
                    keep_data_out{i} = [keep_data_out{i}, keep_data{i}];
                end
                assignCost_out(i) = assignCost_out(i) + assignCost(i); % dummy
            end
        end
    end
    % save mat data
